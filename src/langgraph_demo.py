...
def run_demo():
    ...
    def agent_node(state: AgentState):
        ...
        if not llm:
            ...
            if isinstance(last_message, ToolMessage):
                return {"messages": [AIMessage(content=f"Tool execution completed. Output: {last_message.content}")]}

            content = last_message.content.lower()
            if "git" in content:
                print("  [Agent Decides]: Call check_git_status")
                return {"messages": [AIMessage(content="", tool_calls=[{"name": "check_git_status", "args": {}, "id": "call_1"}])]}
            elif "spark" in content or "sales" in content:
                print("  [Agent Decides]: Call query_spark_sql")
                return {"messages": [AIMessage(content="", tool_calls=[{"name": "query_spark_sql", "args": {"query": "SELECT * FROM sales"}, "id": "call_2"}])]}
            elif "wiki" in content:
                 print("  [Agent Decides]: Call wikipedia")
                 return {"messages": [AIMessage(content="", tool_calls=[{"name": "wikipedia", "args": {"query": "LangChain"}, "id": "call_3"}])]}
            else:
                return {"messages": [AIMessage(content="I am a demo bot. Ask me about git, spark, or wiki.")]}
    ...
    builder.add_edge("tools", "agent") # Return to agent after tool to possibly say something else
